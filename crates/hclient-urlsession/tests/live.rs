...
/// separates this backend from `hclient-fetch`.
///
...
fn the_capabilities_match_the_configuration() {
    ...
    assert!(
        !c.owns_cookie_jar,
        "the session's cookie storage is nil, so hclient's own jar is in force"
    );
...
/// is what `RedirectSupport::Transparent` lets it do. `hclient-fetch`
...
fn the_clients_redirect_policy_is_what_follows_the_hop() {
    ...
    let client = hclient::Client::builder(UrlSession::new())
        .build()
        .expect("build");
