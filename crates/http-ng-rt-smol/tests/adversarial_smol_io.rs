...
//! `crates/http-ng-rt-tokio/tests/adversarial_tokio_io.rs`), itself adapted
...
//! all 7 tests passed against `http-ng-rt-smol` at `71cad8d`. Confirmed
//! non-vacuous the same way Task 3's suite was: temporarily dropping
//! `http-ng-rt`'s `FuturesIo::poll_read`'s `.min(self.scratch.len())` made
...
//! = false }` as a **dev-dependency** of `http-ng-rt-smol` (it is only a
//! normal, non-dev dependency of `http-ng-rt`, so it is not visible to an
//! integration test under `crates/http-ng-rt-smol/tests/` without this) -
//! drop this file into `crates/http-ng-rt-smol/tests/` in a scratch clone,
//! add that dev-dependency, and `cargo test -p http-ng-rt-smol --test
//! adversarial_smol_io --all-features`.
use http_ng_rt::{FuturesIo, TcpAdoptStd, TcpConnect, TcpOpts};
...
const SCRATCH: usize = 8 * 1024; // must match the private const in http-ng-rt's futures_io.rs
