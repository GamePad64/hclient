...
//!
//! `begin_connect`/`build_socket` are private to `http-ng-rt-smol::lib`, so
...
//! review: both tests below passed, repeatably, against `http-ng-rt-smol`
...
//!
//! To re-run: drop this file into `crates/http-ng-rt-smol/tests/` in a
//! scratch clone and `cargo test -p http-ng-rt-smol --test
