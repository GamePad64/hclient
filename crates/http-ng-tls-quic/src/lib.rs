...
//!
//! # Why this is its own crate rather than a feature of `http-ng-tls`
//!
//! Cargo unifies features across a dependency graph. A `quic` feature on
//! `http-ng-tls` would put `quinn-proto` into the graph of every build in
...
//!
//! `http-ng-tls-rustls` gains one implementation behind its own `quic`
//! feature. `http-ng-tls-native-tls` gains **nothing at all, and implements
...
//!
//! [`TlsConnect`]: http_ng_tls::TlsConnect
//! [`NoTls`]: http_ng_tls::NoTls
#![forbid(unsafe_code)]
...
pub struct QuicTlsRequest<'a> {
    ...
    ///
    /// [`TlsConnect::reports_alpn`]: http_ng_tls::TlsConnect::reports_alpn
    pub alpn: &'a [&'a [u8]],
    ...
    ///
    /// [`TlsRequest::ech`]: http_ng_tls::TlsRequest::ech
    pub ech: Option<&'a [u8]>,
    ...
    ///
    /// [`TlsRequest::early_data`]: http_ng_tls::TlsRequest::early_data
    pub early_data: bool,
}
