...
use http_ng_cookie::{CookieJar, Limits, NoList, Rejected};
...
fn without_a_list_every_domain_attribute_is_refused_rather_than_guessed() {
    ...
    assert_eq!(jar.len(), 2);
    assert!(jar.iter().all(http_ng_cookie::Cookie::host_only));
}
...
fn the_bound_is_enforced_per_domain_and_evicts_the_least_recently_used() {
    ...
    let names: Vec<_> = jar.iter().map(http_ng_cookie::Cookie::name).collect();
    assert!(
        !names.contains(&"a"),
...
fn the_total_bound_is_enforced_across_domains() {
    ...
    assert_eq!(jar.len(), 2);
    let domains: Vec<_> = jar.iter().map(http_ng_cookie::Cookie::domain).collect();
