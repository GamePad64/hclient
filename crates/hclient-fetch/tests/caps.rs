...
/// `hclient-native` went on declaring `Configurable` for three more
/// versions. `Internal` is `RedirectSupport`'s own name,
/// in `hclient-core`, for exactly this shape — its doc comment names a
...
fn the_cheap_presence_check_and_the_deciding_probe_still_agree() {
    assert_eq!(
        hclient_fetch::testing::supports_duplex_for_test(),
        hclient_fetch::testing::supports_streaming_request_body_for_test(),
...
fn undeclared_capability_fields_match_their_conservative_defaults_today() {
    let hclient_core::Capabilities {
        ...
        informational_1xx,
        ..
    } = hclient_fetch::Fetch::new().capabilities_for_test();
...
fn supports_duplex_reflects_the_prototype_not_a_hardcoded_constant() {
    ...
    let observed = hclient_fetch::testing::supports_duplex_for_test();
