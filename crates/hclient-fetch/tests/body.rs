...
async fn streams_a_response_body_in_chunks() {
    ...
    let body = hclient_fetch::testing::fetch_body(&f, "data:text/plain,hello%20world")
        .await
        .unwrap();
...
async fn microtask_tick() {
    ...
    hclient_fetch::testing::send_js_future(p)
        .await
...
async fn multiple_chunks_are_delivered_as_separate_frames_not_merged() {
    ...
    let mut body = hclient_fetch::testing::body_from_response(&resp).unwrap();
