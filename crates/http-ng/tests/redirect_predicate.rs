...
use http_ng::{Client, RedirectRefused, RedirectVerdict};
...
fn go(c: &Client<MockTransport>) -> Result<http_ng::Collected, http_ng_core::Error> {
...
fn each_verdict_decides_the_hop_and_the_server_sees_the_difference() {
    ...
    assert_eq!(*err.kind(), http_ng_core::ErrorKind::Redirect, "{err:?}");
...
fn a_predicate_against_an_internally_redirecting_backend_is_refused() {
    ...
    caps.redirects = http_ng_core::RedirectSupport::Internal;
