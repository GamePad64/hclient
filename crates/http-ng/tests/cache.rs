...
//!
//! `http-ng-cache` is tested on its own — 62 tests over the policy, the
...
/// `http-ng-fetch` is the backend that triggers it, because the browser
...
fn a_cache_against_a_transport_that_owns_one_is_refused_at_build() {
    ...
    let err = Client::builder(http_ng::mock::MockTransport::new().with_capabilities(caps))
        .cache(HttpCache::new())
        .build()
    ...
    let ok = Client::builder(http_ng::mock::MockTransport::new())
        .cache(HttpCache::new())
        .build();
