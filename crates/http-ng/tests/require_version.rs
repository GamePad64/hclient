...
//!
//! `http-ng-native/tests/require_version.rs` and its `tests/http2.rs`
...
//! `http-ng-fetch` must go on working for every caller who never mentions
//! a version.
//!
//! No target gate: everything here is `http-ng-mock` and no socket, so it
...
fn send(c: &Client<MockTransport>, req: http::Request<RequestBody>) -> Result<u16, http_ng::Error> {
...
///
/// `http-ng-fetch` and `http-ng-wasi` are the real subjects: neither
...
/// client is exactly what most callers of `http-ng-fetch` want.
#[test]
