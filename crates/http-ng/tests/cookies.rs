...
//!
//! `http-ng-cookie` is already tested to death on its own — 95 tests, RFC
...
//! attaches nothing passes every assertion made in `http-ng-cookie` and
...
use http_ng::cookie::CookieJar;
use http_ng::{Client, UnsupportedCapability};
...
/// `RedirectSupport::Internal` earned its variant. `http-ng-fetch` is the
