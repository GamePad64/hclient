...
//! bounded loop `http-ng-fetch` and `http-ng-wasi` use, so a rewindable
...
//! [`RetryKind`]: http_ng_core::RetryKind
...
pub enum MultipartError {
    ...
    ///
    /// The same bound, for the same reason, as `http-ng-fetch`'s and
    /// `http-ng-wasi`'s: a factory that always rewinds to a factory would
...
/// `http-ng-fetch`'s and `http-ng-wasi`'s: this names how many times the
...
impl Form {
    ...
    /// driving a [`Transport`](http_ng_core::unversioned::Transport)
...
/// reasoning is on `RewindFactory` in `http-ng-core`), and a part body has
...
/// bounded loop `http-ng-fetch` and `http-ng-wasi` use for the whole
/// request body.
///
...
mod tests {
    ...
    /// The bound `http-ng-fetch` and `http-ng-wasi` already carry, at the
