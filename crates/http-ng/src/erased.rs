...
//!
//! `http_ng_cookie::CookieJar<P>` and `http_ng_cache::HttpCache<S>` are
//! generic, and until this module existed `http-ng` accepted only their
//! defaulted forms — so a caller who wanted a disk-backed cache, a store
//! shared between processes, or a jar over
//! [`NoList`](http_ng_cookie::NoList) could reach it in `http-ng-cache` or
//! `http-ng-cookie` and not through the facade. The seam existed one crate
...
//! `http-ng-cookie` and `http-ng-cache` are **optional dependencies**.
//! `Client<T, Tm, P = http_ng_cookie::BuiltinList, S = ..>` names two
...
#[cfg(feature = "cookies")]
pub struct AnyList(
    Box<dyn http_ng_cookie::PublicSuffixList + Send>, // send-bound-exception: amendment-C12
);
...
#[cfg(feature = "cookies")]
impl http_ng_cookie::PublicSuffixList for AnyList {
...
/// A [`CacheStore`](http_ng_cache::CacheStore) of any type.
///
...
#[cfg(feature = "cache")]
pub struct AnyStore(
    Box<dyn http_ng_cache::CacheStore + Send>, // send-bound-exception: amendment-C12
);
...
#[cfg(feature = "cache")]
impl http_ng_cache::CacheStore for AnyStore {
    fn get(&self, key: &http_ng_cache::Key) -> Vec<http_ng_cache::StoredResponse> {
        self.0.get(key)
    }
    fn put(&mut self, key: &http_ng_cache::Key, entry: http_ng_cache::StoredResponse) {
        self.0.put(key, entry);
    }
    fn remove(&mut self, key: &http_ng_cache::Key, selector: &http_ng_cache::Selector) {
        self.0.remove(key, selector);
    }
    fn invalidate(&mut self, key: &http_ng_cache::Key) {
