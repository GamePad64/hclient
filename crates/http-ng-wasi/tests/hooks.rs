...
/// where the clock is read, and nowhere else is; `http-ng-native` and
/// `http-ng-h3` get that for free by counting reads through a `Timer` they
/// were handed, and `http-ng-fetch` by replacing the browser's clock.
...
///
/// The count is exact for the same reason `http-ng-h3`'s clock counts are:
