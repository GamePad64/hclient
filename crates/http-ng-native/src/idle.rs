...
//!
//! It is the same measurement `http_ng::Deadline`'s doc comment records,
...
//! because [`http_ng_core::unversioned::Timer`] carries an associated
...
/// `http_ng::TotalTimeoutElapsed` is one: a caller must be able to tell
...
/// same reasoning, in the same words, as `http_ng::Deadline`'s: the
...
/// Hand-written for the reason `http_ng::Deadline`'s is: `#[derive(Debug)]`
    ...
    Tm: Timer,
{
    ...
    fn poll_frame(
        ...
        let Some(inner) = this.inner.as_mut() else {
            ...
            // same reason, as `http_ng::Deadline`'s.
        ...
        // discard it. This is the same ordering rule `http_ng::within`
