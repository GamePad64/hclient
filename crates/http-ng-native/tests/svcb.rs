...
//! `http-ng-tls-rustls/tests/ech.rs`, which is where the technique — and
...
//! see `http_ng_native`'s `discovery` module doc), so every URI below is
...
use http_ng_native::{Native, SVCB_FAILURE_TTL};
...
impl Resolve for FakeDns {
    ...
    fn lookup_svcb(
        &self,
        _name: &str,
    ) -> impl futures_util::Stream<Item = Result<SvcbEndpoint, http_ng_core::Error>> {
...
///
/// The runtime seam is how time reaches `http-ng-native` — `Native`'s
...
/// **The trap this whole item had to settle first.** `http-ng-tls-rustls`
/// refuses a non-`None` `TlsRequest::ech` before a byte reaches the wire
/// (v0.3 W2 part 1, and `http-ng-tls-rustls/tests/ech.rs` measures exactly
...
/// speaks TCP, `http-ng-h3` is a different crate with different bounds,
