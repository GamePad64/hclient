//! `RequireVersion` on `http-ng-native`, watched from the server's side of
...
use http_ng::{Client, RequireVersion, VersionNotAvailable};
...
fn plain(url: &str) -> http::Request<http_ng_core::RequestBody> {
...
fn refusal(e: &http_ng::Error) -> VersionNotAvailable {
    assert_eq!(
        *e.kind(),
