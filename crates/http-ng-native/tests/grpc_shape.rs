...
//! measures whether that shape is reachable through `http_ng::Client` over
//! `http-ng-native`.
//!
...
/// (see `http_ng::Response::chunk`'s doc comment, and the test below that
...
fn call(server: &Fixture, path: &str, body: RequestBody) -> http::Request<RequestBody> {
    http::Request::builder()
        ...
        .header("user-agent", "grpc-rust-http-ng/0.1.0")
        .body(body)
        .unwrap()
}
...
async fn the_call_definition_reaches_the_wire_te_trailers_included() {
    ...
    assert_eq!(s.header("user-agent"), Some("grpc-rust-http-ng/0.1.0"));
    assert!(
...
async fn an_idle_stream_survives_by_default_and_is_cut_only_when_asked() {
    ...
    assert_eq!(
        err.kind(),
        &http_ng_core::ErrorKind::Timeout(http_ng_core::Phase::BetweenBytes),
