...
//!
//! `docs/v04-design.md`'s Appendix B proposed making `http-ng-native`
//! enforce a `false` the way `http-ng-h3` does (a typed
...
//! `http_ng_native::UndeclaredRequestTrailers` now, and the three tests
...
use http_ng_native::{Native, UndeclaredRequestTrailers};
...
async fn exchange(
...
) -> (Result<u16, http_ng_core::Error>, String) {
    ...
    let body = http_ng_core::RequestBody::Streaming(Box::new(frames));
...
/// `http_ng_core::Error`, the way a caller would.
fn undeclared(e: &http_ng_core::Error) -> &UndeclaredRequestTrailers {
    assert_eq!(
        e.kind(),
        &http_ng_core::ErrorKind::Body,
        "the request body is what failed, and `Unsupported` is \
         `http-ng-h3`'s answer for a transport that cannot send trailers \
...
mod over_http2 {
    ...
    async fn sends_request_trailers_on_http2_without_any_declaration() {
        ...
        let c = Client::builder(t).build().unwrap();
        let body = http_ng_core::RequestBody::Streaming(Box::new(DataThenTrailers::new(Some(
            "grpc-status",
        ))));
        let resp =
