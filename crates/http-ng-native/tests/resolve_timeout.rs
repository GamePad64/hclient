...
use http_ng_core::{ErrorKind, Phase, RequestBody, Timeouts};
use http_ng_dns::{Resolve, ResolvedAddr, SvcbEndpoint};
use http_ng_native::{Native, ResolveTimedOut};
...
fn go<D: Resolve + Clone>(dns: D, port: u16, t: Timeouts) -> Result<u16, http_ng_core::Error> {
...
impl Resolve for Hinting {
    ...
    fn lookup_svcb(
        &self,
        _name: &str,
    ) -> impl Stream<Item = Result<SvcbEndpoint, http_ng_core::Error>> {
