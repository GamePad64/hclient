...
/// `http_ng_tls::TlsRequest::server_name`, whose doc says so at the seam
...
/// already has. `http-ng-native` and `http-ng-h3` both hold a `Uri` and
/// both feed a TLS seam; `http-ng-dns` and `http-ng-dns-doh` both parse
/// literals; `http-ng-tls`, whose doc has to name the duty, depends on
/// this crate and not on any of them. Putting it in `http-ng-dns` would
/// make a TLS server name reach through a resolver crate for a fact about
/// URI syntax, and putting it in `http-ng-tls` would do the mirror image
/// to a resolver.
///
/// # What it does not do
///
