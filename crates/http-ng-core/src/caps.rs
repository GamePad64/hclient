...
pub enum RedirectSupport {
    ...
    ///
    /// Резолюция review (Task 16, находка B-9): раньше здесь примером стоял
    /// `wasi:http`, что неверно — измерено на живом хосте (wasmtime),
    /// что 3xx-ответ доходит до гостя как есть, гость обязан обрабатывать
    /// редирект сам. Поэтому `WasiHttp` заявляет `RedirectSupport::None`, а
    /// не `Internal` (`crates/http-ng-wasi/src/lib.rs`). Пример, для
    /// которого этот вариант уместен — не в этом workspace: браузерный
    /// `fetch()` с `redirect: "follow"` (поведение по умолчанию) следует
    /// редиректу внутри браузера, а JS-код видит только финальный ответ, не
    /// имея возможности перехватить промежуточные хопы.
    Internal,
    /// Мы задаём политику.
    Configurable,
