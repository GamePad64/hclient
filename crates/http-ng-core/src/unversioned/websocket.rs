...
//!
//! The same reasoning `http-ng-tls-quic`'s `QuicTlsConnect` rests on, and
//! `http_ng_rt::TcpAdoptStd` before it: the intersection between "send a
...
pub trait WebSocketConnect {
    ...
    /// fail rather than drop it.** That is the rule `http-ng-wasi` already
