...
//!
//! # Why this file rather than a copy of `http-ng-rt-tokio/tests/udp.rs`
//!
...
async fn udp_pair_property_holds_for_tokio() {
    exercise_udp(http_ng_rt_tokio::Tokio, "tokio").await;
}

#[test]
fn udp_pair_property_holds_for_smol() {
    futures_executor::block_on(exercise_udp(http_ng_rt_smol::Smol, "smol"));
}
...
async fn both_backends_report_the_same_kernel() {
    ...
    let t = http_ng_rt_tokio::Tokio.bind(v4).expect("bind on tokio");
    let s = http_ng_rt_smol::Smol.bind(v4).expect("bind on smol");
    assert_eq!(
        t.caps(),
        s.caps(),
