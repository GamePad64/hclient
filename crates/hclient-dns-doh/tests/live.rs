...
//! `HCLIENT_LIVE_DOH` is set — which `just test-doh-live` sets and nothing
//! else does. `HCLIENT_REQUIRE_NETWORK` turns every one of those skips into
//! a panic, for a runner that promised connectivity; it is the same shape
//! as `HCLIENT_REQUIRE_WASMTIME` and `HCLIENT_REQUIRE_TUNTAP`, and for the
...
const OPT_IN: &str = "HCLIENT_LIVE_DOH";

/// Set by a job that has promised outbound connectivity. Turns every skip
/// below into a panic — the shape `HCLIENT_REQUIRE_WASMTIME` established.
const REQUIRE_MARKER: &str = "HCLIENT_REQUIRE_NETWORK";
...
const LIVE_TIMEOUTS: hclient_core::req::Timeouts = hclient_core::req::Timeouts::new()
...
///
/// The line `hclient_dns`'s module doc draws — "asked and found nothing" is
