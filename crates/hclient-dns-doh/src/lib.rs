...
//! everything [`hclient_core::Timeouts`] can express — there is no `total`
...
/// `IpLiteralOnly::literal` documents in `hclient-dns`.
    ...
    F: Resolve,
{
    ...
    async fn addrs(&self, name: &str, family: Family) -> Vec<Result<ResolvedAddr, Error>> {
        ...
        // `hclient_native::connect` hands `Uri::host()` to the resolver
