...
Same convention as `crates/hclient-dns-doh/mutations.py`: each entry names
...
Two families of mutation, and both are needed:
  ...
  * M1-M3 remove the strip at one call site each. They say the fix is
    ...
    by different tests — that the two `hclient-h3` sites are separately
...
some other crate is visible rather than silently attributed.
    ...
    python3 crates/hclient-core/bare-host-mutations.py        # all
    python3 crates/hclient-core/bare-host-mutations.py M4     # one
