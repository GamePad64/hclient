...
//! `hclient-tls`'. What lives here is what a *request* says, which is why
//! it is an `http::Extensions` type and not a trait: it travels with the
//! request, and any transport in the graph can read it — which is why a
...
/// caller invented.
///
/// A request extension: a per-request choice the transport reads. **A
/// label and never a credential** — extensions reach `Transport::execute`
/// and are readable by any transport.
///
...
///
/// **The field is private and the representation is not promised.**
/// [`Self::name`] is the whole of the read side.
