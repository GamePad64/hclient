...
///
/// Four independent toggles, one per httpie `--print` letter — not four
...
// Each `bool` field below is one independent `clap` flag — `--verbose`,
// `--json`, `--follow` and so on — so the count here is the count of flags
