...
//!
//! So [`Connected`](hclient_core::hooks::Connected),
...
//! classified `hclient_core::error::Error` through `convert::wasi_err`, and
//! reporting one as `CloseReason::Failed` would announce the end of a
//! connection whose beginning was never announced, under
//! [`ConnectionId::UNWATCHED`](hclient_core::hooks::ConnectionId::UNWATCHED),
...
//!   [`Event`](hclient_core::hooks::Event) and no capabilities, so
...
pub(crate) fn mark<H: Hooks>() -> Option<Instant> {
    hclient_core::hooks::mark::<H, _>(Instant::now)
}
...
pub(crate) fn since(at: Option<Instant>) -> Duration {
    hclient_core::hooks::since(at, |t| Instant::now().saturating_duration_since(t))
}

#[cfg(test)]
mod tests {
    use super::*;
    use hclient_core::hooks::{Event, NoHooks};
    ...
    /// clocks in `hclient-native` and `hclient-h3` measure.
    ///
