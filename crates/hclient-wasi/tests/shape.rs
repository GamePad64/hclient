//! Assertions about `hclient-wasi`'s public API shape, kept outside `src`
//! — the same trick as `hclient-core/tests/shape.rs` (see its doc
...
fn a_non_send_hook_still_gives_a_working_transport() {
    ...
    let transport = hclient_wasi::WasiHttp::new().hooks(seen.clone());
    let req = http::Request::builder()
        .uri("http://example.invalid/")
        .body(RequestBody::Empty)
        .unwrap();
...
fn the_no_op_hook_takes_up_no_room_in_the_transport() {
    ...
    assert_eq!(
        ...
        std::mem::size_of::<hclient_wasi::WasiHttp>(),
    );
}
