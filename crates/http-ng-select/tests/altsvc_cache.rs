...
//! as an argument, exactly as it does on `http-ng-native`'s negative cache
...
use http_ng_select::altsvc::{AltSvcCache, FieldValue, Origin, parse};
...
/// than by a sweep — `http-ng-native`'s model, and the reason is that this
