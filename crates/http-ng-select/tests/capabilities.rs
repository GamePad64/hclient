...
//! **actual** capability sets `http-ng-native` and `http-ng-h3` report in
...
use http_ng_select::{Selecting, combine};
...
async fn the_two_stacks_disagree_on_exactly_seven_fields_today() {
    ...
    assert_eq!(
        ...
        "the measured disagreement between http-ng-native and http-ng-h3 has moved; \
...
async fn the_stored_answer_holds_whichever_stack_serves_the_request() {
    ...
    // `http-ng` reads this field, so `None` would not stop a marked request
...
/// one-connection-per-request behaviour) and `http-ng-h3` shares
...
/// `http-ng-core` can be made exhaustive and there is no compile error when
...
fn every_capability_field_is_accounted_for_and_a_new_one_fails_this_test() {
    ...
    assert_eq!(
        ...
        "`Capabilities` has changed shape; `http_ng_select::combine` must say \
