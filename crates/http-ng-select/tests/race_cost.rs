...
fn start_tcp(
    ...
    // `http/1.1` alone: with `http-ng-native`'s `http2` feature on — which
...
///    us"*, `http-ng-rt`'s `caps.rs` — so [`tcp`] above connects with
...
fn tcp_on_unit_runtime(
    cert: &rustls::pki_types::CertificateDer<'static>,
    nodelay: bool,
) -> Native<http_ng_rt_tokio::Tokio, Tls, FakeDns> {
    Native::new(http_ng_rt_tokio::Tokio, client_tls(cert), FakeDns::new())
        .tcp_opts(http_ng_rt::TcpOpts {
            nodelay,
            ..Default::default()
        })
...
///
/// `Error = http_ng_core::Error` rather than the trait's own
...
where
    T: Transport<Error = http_ng_core::Error>,
...
/// confirms it for *this* shape: `http_ng_h3::H3::execute` with no
...
/// `http-ng-h3` claims it. What is measured here is whether it holds
...
///
/// This is **not** what a race in `http-ng-select` would look like — there
