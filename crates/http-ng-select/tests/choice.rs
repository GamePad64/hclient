...
use http_ng_core::{RequestBody, RequireVersion};
use http_ng_h3::H3;
...
/// `http-ng-native`'s own selection wrote the skip down for.
#[tokio::test(flavor = "multi_thread")]
...
/// `http_ng::Client`, two origins' worth of behaviour, no `#[cfg]` and no
/// second client.
///
...
async fn one_client_reaches_both_servers_depending_only_on_the_record() {
    ...
    for (record, expected, quic_delta, tcp_delta) in [
        ...
        let client = http_ng::Client::builder(selector(&pair, dns))
            .build()
