...
//! cache *deliberately*, and gives `http-ng-native`'s reason: *"this origin
...
//! now()`, for the reason `http-ng-native`'s negative cache gives for its
...
//! `http_ng_native::discovery::NegativeCache::suppressed` — so the cache
...
//! no record can cross the `Transport` seam, so `http-ng-native` fetches
...
impl Origin {
    /// The host is ASCII-lowercased for the reason `http-ng-native`'s pool
...
/// VPN connecting; nothing in `http-ng-rt` carries one, and inventing one
...
impl AltSvcCache {
    ...
    /// `http-ng-native`'s reason: this is the only place that asks, an
        ...
        match found {
            Some(a) => {
                entries.insert(
                    origin.clone(),
                    Entry {
                        // Saturating for `http-ng-native`'s reason: an
