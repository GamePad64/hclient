...
//! is the memory of what happened when we believed it. `http-ng-native`'s
...
//! failure is `http-ng-h3`, which has no failure memory of any kind, and
//! could not usefully have one, because it is not the thing that would act
//! on it. `http-ng-native`'s own `NegativeCache` is a **different fact** —
...
//! origin, where `http-ng-native`'s own negative cache costs none because
...
//! idempotence judgement — and `http-ng-native`'s sentence is true of it
...
/// HTTP/3.
///
/// Five minutes, and it is `http_ng_native::SVCB_FAILURE_TTL`'s number
...
/// `AltSvcCache` and `http-ng-native`'s `NegativeCache` both have, which is
