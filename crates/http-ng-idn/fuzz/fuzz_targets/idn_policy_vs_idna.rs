...
//! `http_ng_idn::testing::policy_over` lets the backend be *supplied*.
...
/// The oracle, and the backend, and the same call `http-ng-proto` makes.
...
fuzz_target!(|data: &[u8]| {
    ...
    let ours = http_ng_idn::testing::policy_over(idna_says, &domain);
