...
with `HTTP_NG_REQUIRE_NETWORK` set. The recipe MUST fail. A `SURVIVED` here
...
hermetic suite (`cargo nextest run -p http-ng-dns-doh` with the opt-in
...
project a commit containing three live mutations.
    ...
    python3 crates/http-ng-dns-doh/live-mutations.py        # all of them
    python3 crates/http-ng-dns-doh/live-mutations.py H      # one group
    python3 crates/http-ng-dns-doh/live-mutations.py L4     # one mutation
"""
...
LIVE = ROOT / "crates/http-ng-dns-doh/tests/live.rs"
JUSTFILE = ROOT / "justfile"
DOH_SRC = ROOT / "crates/http-ng-dns-doh/src"
...
def hermetic():
    ...
    env = {k: v for k, v in os.environ.items() if k not in ("HTTP_NG_LIVE_DOH", "HTTP_NG_REQUIRE_NETWORK")}
    return subprocess.run(
        ["cargo", "nextest", "run", "-p", "http-ng-dns-doh", "--no-fail-fast"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        env=env,
...
def live():
    ...
    env = dict(os.environ, HTTP_NG_REQUIRE_NETWORK="1")
    return subprocess.run(
        ["just", "test-doh-live"],
        cwd=ROOT,
        capture_output=True,
