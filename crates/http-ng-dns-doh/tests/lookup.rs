...
//! yielded.
//!
//! The line these tests exist to defend is the one `http_ng_dns`'s module
...
type Item = Result<ResolvedAddr, http_ng_core::Error>;
...
fn one_error(items: Vec<Item>) -> http_ng_core::Error {
    let mut items = items;
...
/// the operation the caller asked for. `http-ng-native`'s connector reads
