//! `hclient-rt` capabilities implemented on top of embassy: the clock is
...
//! `hclient-dns-doh`'s example is not: it would cost a dev-dependency on
//! `hclient-native` and `hclient` to compile one line. This crate's own
//! half is checked.
//!
...
//!    `hclient-native`** — and it would leave the capability lying for
...
//!    (`tests/tuntap.rs`) checks it the same way `hclient-native`'s own
...
fn no_family(family: &str, feature: &str) -> std::io::Error {
    std::io::Error::new(
        ...
        format!(
            "this build of hclient-rt-embassy has no {family} support: \
             enable the `{feature}` feature of hclient-rt-embassy"
        ),
    )
}
