...
//! `http-ng-native`'s.** There the fresh-connection figure is `1`, because
...
//! `std::time::Instant` (see `http_ng_quinn`'s `until`), not through this
//! clock.
//!
...
use http_ng_core::unversioned::{Event, Hooks, NoHooks, Transport};
use http_ng_dns::IpLiteralOnly;
use http_ng_h3::{H3, QuinnTask};
...
fn the_no_op_hook_takes_up_no_room_in_the_transport() {
    ...
    assert_eq!(
        std::mem::size_of::<H3<TokioHandle, http_ng_tls_rustls::Rustls, IpLiteralOnly, NoHooks>>(),
        std::mem::size_of::<H3<TokioHandle, http_ng_tls_rustls::Rustls, IpLiteralOnly>>(),
