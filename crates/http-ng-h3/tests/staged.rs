...
//!
//! Three claims, and the difference from `http-ng-native`'s file of the
...
//! - **a handle nobody spends needs no `Drop`.** `http_ng_native::Staged`
...
/// from `http-ng-native`'s with the same observable end.
///
...
/// whole, which is the property the fallback in `http-ng-select` is built
