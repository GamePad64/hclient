...
//!
//! The counterpart of `crates/http-ng/tests/two_runtimes.rs`, which does
...
//! under `http_ng_rt_tokio::TokioHandle` inside a real `tokio::runtime::
//! Runtime`, once under `http_ng_rt_smol::Smol` on a bare
...
//! and it is the same probe `http-ng-rt-pair-check`'s `pair_property.rs`
//! and `http-ng`'s `two_runtimes.rs` already use — so a green run here is
...
fn http3_over_a_real_socket_under_tokio() {
    ...
    let bodies = rt.block_on(async {
        let handle = http_ng_rt_tokio::TokioHandle::current().expect("inside block_on");
...
fn http3_over_a_real_socket_under_smol() {
    ...
    // own business.
    let s = server::start(Behaviour::Echo);
    let bodies = futures_executor::block_on(fetch_once(http_ng_rt_smol::Smol, &s.cert_der, s.addr));
    assert_eq!(bodies, 2);
    assert_eq!(s.requests(), 2);
    assert_eq!(
        s.accepted(),
