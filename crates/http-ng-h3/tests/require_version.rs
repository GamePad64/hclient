...
//!
//! It is *which* demands it refuses. `http-ng-h3` always speaks HTTP/3, so
...
//!   `VersionNotAvailable` `http-ng-native` raises, so a caller who swapped
...
use http_ng_core::{ErrorKind, RequestBody, RequireVersion, VersionNotAvailable};
...
fn refusal(e: &http_ng_core::Error) -> VersionNotAvailable {
    assert_eq!(
        *e.kind(),
        ErrorKind::Unsupported,
        "the refusal is `Unsupported`, matching `http-ng-native`'s: {e}"
    );
...
/// `http-ng-core` for why a one-version transport reporting `false` would
