...
//! `http-ng-wasi`'s module doc records the same consequence for the same
...
//! body's terminal error. This is cost (2) from `http-ng-wasi`'s deferral
...
/// arm above.
///
/// The recursion is the same shape `http_ng_native::body::Inner::
...
/// `http-ng-native`'s h2 pump has to build by hand with
...
#[error(
    "http-ng-h3 does not send request trailers; Capabilities::request_trailers reports false, \
