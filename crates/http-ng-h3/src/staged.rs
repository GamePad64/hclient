...
//! `http-ng-native`'s side and left this crate's half open: *"whether `H3`
...
//!
//! This is the honest difference from `http_ng_native::Staged`, and it is
...
//! `http_ng_native::Staged` *owns* the connection it took out. Both satisfy
...
//! document left open:
//!
//! - **A dropped handle needs no `Drop`.** `http-ng-native` had to give
...
//!
//! `http-ng-h3` makes no HTTPS-record lookup at all, so there is nothing
...
use http_ng_core::unversioned::{ConnectTiming, Connected, Event, Hooks, Reused, Transport};
use http_ng_core::{Error, ErrorKind, RequestBody, Timeouts, check_version};
use http_ng_tls_quic::QuicTlsConnect;
use std::fmt;
...
///
/// The same shape `http_ng_native::StagedConnect` has, declared separately
...
/// and `http-ng-fetch` declares `timeouts.connect = false` because
    ...
    H: Hooks + Clone,
{
    ...
    pub(crate) fn admit(&self, req: &http::Request<RequestBody>) -> Result<Admitted, Error> {
        ...
        if req
            ...
            .is_some()
            && self.caps.early_data == http_ng_core::EarlyDataSupport::None
        {
            return Err(crate::early::refuse_early_data("http-ng-h3"));
        }
    ...
    pub(crate) async fn stage(
        ...
        //
        // The same scope `http-ng-native` gives `connect`, deliberately: a
