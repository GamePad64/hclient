...
//! `docs/v03-acceptance.md` rather than fixed by editing `http-ng-core`
...
//!
//! `http-ng-native` reports at most once per body, and that is at most
...
/// `crates/http-ng-h3/tests/hooks_cost.rs` counts the reads from outside,
/// on a runtime whose clock reports how often it was asked.
///
/// The same two functions `http-ng-native` has, written again rather than
...
impl ConnState {
    ...
    /// rules `http_ng_core::unversioned::hooks`'s module doc states.
...
/// `H3Body` stays `Unpin` for every `H`, which `http-ng-select` requires of
/// it.
