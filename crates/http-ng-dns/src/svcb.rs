...
//! `http-ng-dns-system`, where its own doc comment gave the reason it
...
//! one crate (`res_query` and `DnsQuery_UTF8`). `http-ng-dns-doh` is a
...
///
/// `dohpath` is worth a second sentence now that `http-ng-dns-doh` exists:
...
/// can drift from another: `http-ng-dns-system`'s `windows.rs` fills this
/// in from an OS-parsed `DNS_SVCB_DATA`, its `svcb.rs` fills it in from a
/// `dns-message-parser` `ServiceBinding`, `http-ng-dns-doh` fills it in
...
/// into its own enum at the call site (`http-ng-dns-system` into
/// `SvcbLookupError::MandatoryKeyAbsent`, `http-ng-dns-doh` into
...
/// `dns-message-parser` — `http-ng-dns-system`'s `res_query` path and
/// `http-ng-dns-doh` — goes through this, so the ECH note below is written
/// down once instead of once per backend. `http-ng-dns-system`'s Windows
...
pub fn binding_from_decoded(binding: &dns_message_parser::rr::ServiceBinding) -> RawBinding {
    ...
    let params = binding
        ...
        .map(|parameter| match parameter {
            ...
            // `http-ng-dns-system`'s `windows.rs` adds nothing. Storing the
