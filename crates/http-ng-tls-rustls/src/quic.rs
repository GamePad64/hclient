...
//! [`TlsConnect`](http_ng_tls::TlsConnect), which has three — the only one
...
impl QuicTlsConnect for Rustls {
    fn quic_client_config(
        ...
        if req.ech.is_some() {
            ...
            return Err(Error::new(
                ErrorKind::Tls,
                std::io::Error::other(
                    "http-ng-tls-rustls does not yet apply an ECH config on the QUIC path; \
    ...
    /// of a connector. See `http_ng_h3`'s early-data module.
