...
//! members here already produce `hclient_core::Error` — classified, with
//! `ErrorKind::Timeout(Phase::BetweenBytes)` off `hclient-native`'s idle
...
pub enum SelectedBody<A, B> {
    /// `hclient-native` answered: HTTP/1.1, or HTTP/2 where its `http2`
    /// feature is on and ALPN chose it.
    Tcp(A),
    /// `hclient-h3` answered: HTTP/3 over QUIC.
    Quic(B),
}
