//! Apple's `URLSession` behind [`Transport`](http_ng_core::unversioned::Transport).
//!
...
//!   — `http-ng-cookie`, `http-ng-cache` and `http_ng_proto::redirect` —
...
//!   porting from `http-ng-native` would lose two features by changing
//!   one line.
