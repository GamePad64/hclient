...
pub(crate) enum SvcbLookupError {
    ...
    Malformed(dns_message_parser::DecodeError),
...
impl std::fmt::Display for SvcbLookupError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::NameNotUsable { name } => {
                write!(f, "`{name}` cannot be used as a DNS query name")
            }
            Self::NoResponse => write!(f, "no configured resolver answered"),
            Self::ResponseCode { rcode } => write!(f, "resolver answered with RCODE {rcode}"),
            Self::Truncated => write!(f, "answer was truncated and no complete one was obtained"),
            Self::LengthUnavailable { ancount } => write!(
                f,
                "resolver reported {ancount} answer record(s) but returned no length to read them with"
            ),
            Self::AnswerTooLarge => write!(f, "answer exceeds the largest possible DNS message"),
            Self::HeaderTruncated { got } => {
                write!(
                    f,
                    "answer is {got} bytes, shorter than the 12-byte DNS header"
                )
            }
            Self::Malformed(e) => write!(f, "malformed answer: {e}"),
            Self::WindowsDnsError { code } => {
                write!(f, "the Windows DNS client returned status {code}")
            }
            Self::MandatoryKeyAbsent { key } => write!(
                f,
                "SvcParamKey {key} is listed as mandatory but is not present in the record"
            ),
        }
    }
}

impl std::error::Error for SvcbLookupError {
