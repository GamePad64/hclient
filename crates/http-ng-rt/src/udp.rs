...
//! conversion in `http-ng-h3`, which is cheaper than a runtime seam that
...
pub trait UdpBind {
    ...
    /// `http_ng_h3::H3`'s `where` clause instead, so the compile error lands
    ...
    /// it: `http-ng-native`'s `FakeStream` holds an `Rc<()>` precisely to
