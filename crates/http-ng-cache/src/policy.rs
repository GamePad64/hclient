...
//! the same rule `http-ng-cookie` and `http-ng-proto` run under, and for
...
/// crate over: the only consumer is a `match` in `http_ng::Client` that
...
///
/// Reported rather than swallowed, for the reason `http-ng-cookie`'s
...
/// a rule someone will one day be surprised by. `http_ng::Client` drops
...
/// rule fired.
///
/// [`Rejected`]: https://docs.rs/http-ng-cookie
#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error)]
#[non_exhaustive]
pub enum NotStored {
    /// Only `GET` responses are stored — see [`HttpCache::lookup`].
    ...
    /// heuristic lifetime. See [`HttpCache::storing`].
    ...
    NoKey,
    /// Larger than [`Limits::max_body_bytes`] — by the `Content-Length`
...
/// when the body ends — in `http-ng` those are minutes apart and a caller
...
/// store; this one belongs to whoever built the [`http_ng::Client`] it
/// sits in.
///
...
///
/// [`http_ng::Client`]: https://docs.rs/http-ng
///
/// ```
/// use std::time::{Duration, SystemTime};
/// use http::{HeaderMap, Method, Response, Uri};
/// use http_ng_cache::{HttpCache, Lookup};
///
...
impl<S: CacheStore> HttpCache<S> {
    ...
    /// a `text/event-stream` that never ends, which `http-ng` would then
