...
//! the endpoint's duty and `hclient_core::unversioned::Message` has no
...
//!
//! ```text
//! cargo run -p hclient-tungstenite --example autobahn \
//!     -- ws://127.0.0.1:9001 hclient-tungstenite
//! ```

use futures_util::{SinkExt, StreamExt};
use hclient_core::unversioned::{Message, WebSocketConnect};
...
async fn main() -> std::process::ExitCode {
    ...
    let agent = args.next().unwrap_or_else(|| "hclient-tungstenite".into());
