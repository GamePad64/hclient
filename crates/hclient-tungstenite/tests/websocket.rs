//! The WebSocket seam on `hclient-native`, watched from the other side of
//! a real socket.
//!
...
//! finish at all — `crates/hclient-h3/tests/streaming.rs` is the shape.
//! In particular
...
async fn a_websocket_never_takes_a_pooled_connection() {
    ...
    // than through `hclient::Client`, because `Client` consumes the
...
/// trips. This is the property that differs from `hclient-h3`, where a
...
/// differ in kind rather than in wording. `hclient-fetch` draws the same
