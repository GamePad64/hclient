...
//! `http-ng-native` pays the same duty in the other direction — it refuses
...
//!   `tungstenite` reports to `http-ng-native` as `None`.
...
//! (`http_ng_native::testing::blocking_io`, 600 ms of wall time and 600 ms
//! of CPU). `http-ng-native`'s `Sink` gives real backpressure from the
...
/// `tungstenite` reports to `http-ng-native` as `Message::Close(None)`.
...
/// `http-ng-native`'s `as_http_uri` follows — a caller who already holds an
...
impl Sink<Message> for FetchWebSocket {
    ...
    /// `Stream`. The same decision `http-ng-native`'s `poll_close` makes,
...
/// preference. `http-ng-native` gates its implementation at `websocket`
