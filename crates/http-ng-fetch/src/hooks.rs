...
//! transport that dials its own sockets; `http-ng-h3` was the test of
...
//!
//! [`Connected`](http_ng_core::unversioned::Connected),
//! [`Reused`](http_ng_core::unversioned::Reused) and
//! [`Closed`](http_ng_core::unversioned::Closed) are not emitted, and the
...
//! `http-ng-native`'s refusal to add a *request queued* variant and of
//! `http-ng-h3`'s report that `CloseReason::Ended` has no subject in QUIC:
...
//! [`ConnectTiming`](http_ng_core::unversioned::ConnectTiming) plus
...
//! is the same measurement `http-ng-native` and `http-ng-h3` make through
...
//! [`Timer`](http_ng_core::unversioned::Timer), and it reads
