...
//! `ErrorCode`, and so on — see `http-ng-native/src/body.rs` and
//! `http-ng-wasi/src/body.rs` for the established precedent). This file
...
//! the same category `http-ng`'s `Response::text`/`Response::json` already
...
//! `http-ng-wasi/src/body.rs::size_hint_honoring_end`). This file applies
...
//!
//! `http-ng-wasi`'s `Body::is_end_stream` has to delegate to
