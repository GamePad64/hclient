...
async fn observed_outcome() -> (bool, Option<String>) {
    ...
    let record = http_ng_fetch::testing::send_js_future(outcome)
        .await
...
fn get(url: &str) -> http::Request<http_ng_core::RequestBody> {
...
fn issue_and_poll_once(
    ...
    Box<
        dyn std::future::Future<
                Output = Result<http::Response<http_ng_fetch::Body>, http_ng_core::Error>,
            > + '_,
    >,
> {
    use http_ng_core::unversioned::Transport;
