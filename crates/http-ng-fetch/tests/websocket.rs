//! The browser's WebSocket, behind `http_ng_core::unversioned::
...
use http_ng_core::unversioned::{CloseFrame, Message, WebSocketConnect};
...
async fn open(uri: &str) -> http_ng_fetch::FetchWebSocket {
    Fetch::new()
        .websocket(req(uri))
        .await
...
/// ones RFC 6455's own handshake owns. `http-ng-native` refuses the second
