...
//!
//! This is the honest difference from `hclient_native::staged::Staged`, and it is
...
//! `hclient_native::staged::Staged` *owns* the connection it took out. Both satisfy
    ...
    H: Hooks + Clone + Unpin,
{
    ...
    ) -> Result<http::Response<hclient_core::hooks::Counting<crate::http3::H3Body<H>, H>>, Error>
    {
        ...
        // promise.
        let sent = hclient_core::hooks::meter::<H>(body.size_hint()).map(std::sync::Arc::new);
