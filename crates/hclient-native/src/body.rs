...
//! Rewindable`'s doc comment in `hclient-core`), and such a result must
...
//!
//! `hclient-wasi` reaches the same conclusion one step later — its
...
impl Body for OutgoingBody {
    ...
    fn size_hint(&self) -> SizeHint {
        match &self.inner {
            ...
            // `hclient-core`); a forwarded `Streaming` reports whatever
...
mod tests {
    ...
    // the technique is already used in `hclient-tls::tests::poll_once`):
