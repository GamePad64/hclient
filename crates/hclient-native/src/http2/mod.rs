...
//! v0.3 did the same thing for HTTP/3 (`hclient_h3::pump` is the
...
//! features across a graph, so a library built on `hclient` can never know
...
//! `hclient_h3::pump` and `hclient-wasi`'s module doc record the same
...
/// that is deliberate in the direction [`TcpOpts`](hclient_rt::TcpOpts)
    ...
    I: Read + Write + Unpin,
{
    ...
    /// response that is already on the wire alone. `hclient_h3::H3Body`
...
/// [`hclient_rt::Spawn<F>`](hclient_rt::Spawn) makes the future a type
