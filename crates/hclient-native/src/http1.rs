...
//! `hclient-rt-pair-check`), so this file polls `Connection` **by hand,
...
/// comment and `hclient-rt-pair-check`).
///
...
mod tests {
    ...
    /// choice and the same reasoning as `hclient-native::body::tests::
    ...
    /// — see `hclient-native::body::tests::SinkIo` for the detailed
