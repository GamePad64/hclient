...
//! `hclient_rt_tokio::Tokio` hands the task to an ambient multi-threaded
//! runtime read out of a thread-local; `hclient_rt_smol::Smol` starts a
...
impl Harness for OnSmol {
    type Rt = hclient_rt_smol::Smol;
    fn rt(&self) -> Self::Rt {
        hclient_rt_smol::Smol
    }
    fn run<F: Future>(&self, f: F) -> F::Output {
