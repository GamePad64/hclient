...
//!
//! # Where this differs from `hclient-native`'s file of the same name
//!
//! Three places, and all three are facts about QUIC rather than about the
//! seam — `crates/hclient-h3/src/hooks.rs` argues each one where the code
//! is:
//!
    ...
    fn only_head(&self) -> (u64, u16, Duration) {
        ...
        match &h[0] {
            Seen::Head {
                ...
                // `Option` so that `hclient-fetch` and `hclient-wasi` can
        ...
        let seen = match event {
            ...
            Event::Closed(e) => Seen::Closed {
                ...
                reason: match e.reason {
                    ...
                    hclient_core::hooks::CloseReason::Failed(err) => {
...
///
/// `hclient-native`'s equivalent uses `Native::without_pool()`; this
...
/// to make readable, and the one `hclient-native`'s file records as
...
///
/// `hclient-native` gets this for free: an h2 connection is checked out of
...
async fn a_replayed_0_rtt_request_reports_one_head_and_one_connection() {
    ...
    marked
        .extensions_mut()
        .insert(hclient_core::req::AllowEarlyData);
...
/// one — the same rule `hclient-wasi` reasons its way to from the other
