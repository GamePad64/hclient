...
async fn checkout_walks_past_a_dead_connection_to_a_live_one() {
    ...
    // The size is now stated where it belongs, in `hclient`'s
...
impl hclient_rt::Timer for LateEof {
    type Instant = <Tokio as hclient_rt::Timer>::Instant;
    /// Delegation stays exact: the wrapper's sleep IS the inner clock's.
    type Sleep = <Tokio as hclient_rt::Timer>::Sleep;
...
mod alpn_guard {
    ...
    impl hclient_tls::TlsIdentity for ReportsAlpn {
    ...
    impl hclient_tls::TlsConnect for ReportsAlpn {
        type Stream<S>
            = S
        where
    ...
    /// `hclient-tls-rustls/tests/config_id.rs`.
    ///
