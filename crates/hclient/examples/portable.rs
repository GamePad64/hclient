//! `act`'s `http-client` component, ported to `hclient` — the acceptance
...
//!   with `hclient`, so [`FetchArgs`] here is a plain struct.
...
//!   dependency `hclient` does not already have.
//!
