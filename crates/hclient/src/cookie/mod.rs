...
//! for any transport: the jar stays a pure function of its inputs, and
//! `Client` is what supplies the `now`.
//!
...
//! - a `Domain` that is a public suffix — [`PublicSuffixList`];
...
//!   row, the `localStorage` key — is the caller's.
...
//!   nobody touches keeps its expired entries until it is touched.
