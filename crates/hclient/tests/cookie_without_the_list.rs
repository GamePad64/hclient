...
//! problem for `hclient-proto`'s `idn` feature is already handled.
//!
...
use hclient::cookie::{BuiltinList, CookieJar, PublicSuffixList, Rejected};
...
fn every_domain_attribute_is_refused_and_the_error_names_the_build() {
    futures_executor::block_on(async {
        ...
        assert_matches!(
            jar.store(&at, &header("a=1; Domain=co.uk"), now()).await,
