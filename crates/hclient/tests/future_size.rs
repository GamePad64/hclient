...
//! in another crate.** `hclient-native`'s
...
/// `hclient-native`'s twin of this test then measured it — one extra
...
fn the_execute_future_stays_under_its_ceiling() {
    ...
    assert!(
        ...
        "`Client::execute`'s future is {size} bytes, over the {CEILING}-byte \
         ...
         `hclient-native`'s `checkout_walks_past_a_dead_connection_to_a_live_one` \
