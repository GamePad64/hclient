...
//!
//! `hclient-fetch` lists `Accept-Encoding` among its
...
//! `hclient-mock` and a byte literal, with no `TcpListener` and no
...
use hclient::{Capabilities, Client, DecompressionSupport, ErrorKind};
...
/// reason `hclient-fetch`'s `Body::size_hint` distrusts `Content-Length`.
///
...
fn a_body_that_is_not_the_coding_it_claims_is_a_decode_error() {
    ...
    assert!(
        std::error::Error::source(&err)
            .and_then(|s| s.downcast_ref::<hclient::DecodeFailed>())
