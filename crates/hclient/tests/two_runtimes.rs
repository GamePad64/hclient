...
//! runtime) through the bounds `hclient_rt::{TcpConnect, Timer, Blocking} +
...
//! two tests below instantiate it — once with `hclient_rt_tokio::Tokio`
//! under `tokio::runtime::Runtime`, once with `hclient_rt_smol::Smol`
...
//! `hclient_rt::Timer::Instant`: it breaks `Tokio` (`Instant =
//! tokio::time::Instant`, a wrapper) and doesn't break `Smol` (`Instant =
//! std::time::Instant`), see `hclient-rt-pair-check`'s `pair_property.rs`,
...
//! NEITHER runtime exists (`hclient-rt-tokio` and `hclient-rt-smol` are
...
/// as `hclient_native::connect::tests::bounded_block_on` and
...
fn identical_code_on_tokio() {
    ...
    let text = with_watchdog(|| rt.block_on(fetch_once(hclient_rt_tokio::Tokio, addr)));
    assert_eq!(text, "same");
}
...
fn identical_code_on_smol() {
    let addr = spawn_server();
    let text =
        with_watchdog(|| futures_executor::block_on(fetch_once(hclient_rt_smol::Smol, addr)));
    assert_eq!(text, "same");
}
