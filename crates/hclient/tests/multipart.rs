...
/// needed here is ten lines, which is the same call `hclient-proto`'s
...
/// omitting it is what left `hclient-select`'s Alt-Svc fixture racing a FIN
...
fn a_buffered_form_declares_its_length_and_parses_back_into_its_parts() {
    ...
    assert!(
        s.header("content-type")
            ...
            .starts_with("multipart/form-data; boundary=----hclient-"),
        "{}",
        s.head
    );
    assert_eq!(
