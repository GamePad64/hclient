...
//!
//! The same reason `hclient-cookie` is one, and the same reason
//! `hclient-proto` is. A cache is *used* by `hclient`, but the substance of
...
//! `hclient-core`, not `hclient`, not any transport.
//!
...
//!
//! `hclient_core::Capabilities::owns_cache` has been a `bool` set by one
...
//!   reason `hclient`'s decompressor makes concrete.
