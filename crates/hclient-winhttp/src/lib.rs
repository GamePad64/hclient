...
//! for the list of things a userspace stack cannot reach on Windows: the
//! system proxy, including a PAC script the OS evaluates; enterprise
//! roots and `SChannel` policy; and Windows credential integration
//! (`WinHttpSetCredentials`, not yet wired up here).
#![cfg(windows)]
#![warn(missing_docs)]
...
//! let client = hclient::Client::builder(WinHttp::new()?).build()?;
//! # let _ = client;
//! # Ok(())
//! # }
//! ```
...
//! # Key concepts
//!
//! [`WinHttp`] is the transport, over `WinHTTP`. [`WinHttpBody`] is the
//! response body. [`Protocols`], set through [`WinHttp::protocols`],
//! switches on HTTP/2 and HTTP/3 — off by default, since enabling one
//! changes what every request puts on the wire. [`WinHttpWebSocket`]
//! implements [`WebSocketConnect`](hclient_core::websocket::WebSocketConnect)
//! over `WinHTTP`'s own WebSocket API.
//!
//! # What it deliberately does NOT take from the OS
//!
//! `WinHTTP` will follow redirects and keep cookies for you, and this
//! transport turns both off — the same behaviour `hclient`'s `Client`
//! already implements, so a caller does not lose it by choosing this
//! backend for the OS-owned things it *is* here for. Redirects are
//! reported [`RedirectSupport::Transparent`](hclient_core::caps::RedirectSupport::Transparent),
//! as `hclient-urlsession` reports and `hclient-fetch` cannot.
//! Decompression, unlike its two ambient siblings, is opt-in here —
//! `WINHTTP_OPTION_DECOMPRESSION` is asked for only when `hclient`'s own
//! decoders are not in force — and there is no response cache to turn
//! off: `WinHTTP` has none.
//!
//! # Deliberately not done
//!
//! Streaming request bodies (a typed `Unsupported` error rather than a
//! silent empty body), the rest of `WinHTTP`'s HTTP/2 and HTTP/3 knobs
//! (no caller here can ask for one yet), and Windows authentication
//! (`WinHttpSetCredentials`, driven from a `401` the way `hclient`'s
//! digest support already is one layer up).
//!
...
//! want of a Windows machine. `sys.rs` states the async contract this
//! leans on hardest where the code depends on it, so the next person with
//! a Windows box knows exactly what to check.
//!
//! # Where to go next
//!
//! `hclient::Client` is the portable facade this transport plugs into.
//! `hclient-urlsession` is the equivalent on Apple platforms.

// Maintainer notes (not rendered):
//
// The front page above was trimmed from the essay that used to be here;
// the full original text is archived verbatim below.
//
// Windows' `WinHTTP` behind [`Transport`](hclient_core::transport::Transport).
//
// The fifth ambient backend — after `hclient-wasi`, `hclient-fetch` and
