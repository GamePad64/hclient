...
//! `:41`). `http_ng_rt`'s own [`Timer`], [`Spawn`] and [`UdpBind`] promise
    ...
    R::Socket: fmt::Debug + Send + Sync + 'static, // send-bound-exception: amendment-C10
{
    ...
    fn spawn(&self, future: QuinnTask) {
        http_ng_rt::Spawn::spawn(&self.rt, future);
    }
    ...
    S: UdpDatagrams + fmt::Debug + Send + Sync + 'static, // send-bound-exception: amendment-C10
{
    ...
    fn poll_recv(
        ...
        let mut ours = vec![http_ng_rt::RecvMeta::default(); meta.len()];
...
///
/// `http_ng_rt::EcnCodepoint` is declared rather than re-exported so that
/// `http-ng-rt` carries no QUIC dependency, and this `match` is the whole
...
fn to_quinn_ecn(c: http_ng_rt::EcnCodepoint) -> quinn::udp::EcnCodepoint {
    match c {
        http_ng_rt::EcnCodepoint::Ect0 => quinn::udp::EcnCodepoint::Ect0,
        http_ng_rt::EcnCodepoint::Ect1 => quinn::udp::EcnCodepoint::Ect1,
        http_ng_rt::EcnCodepoint::Ce => quinn::udp::EcnCodepoint::Ce,
    }
}

fn from_quinn_ecn(c: quinn::udp::EcnCodepoint) -> http_ng_rt::EcnCodepoint {
    match c {
        quinn::udp::EcnCodepoint::Ect0 => http_ng_rt::EcnCodepoint::Ect0,
        quinn::udp::EcnCodepoint::Ect1 => http_ng_rt::EcnCodepoint::Ect1,
        quinn::udp::EcnCodepoint::Ce => http_ng_rt::EcnCodepoint::Ce,
    }
}
...
mod tests {
    ...
    fn ecn_survives_the_round_trip_through_both_conversions() {
        // The price of `http-ng-rt` not depending on a QUIC crate is these
        ...
        for c in [
            http_ng_rt::EcnCodepoint::Ect0,
            http_ng_rt::EcnCodepoint::Ect1,
            http_ng_rt::EcnCodepoint::Ce,
        ] {
