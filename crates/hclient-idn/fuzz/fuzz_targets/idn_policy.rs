...
//! 3. **Idempotence** where the conversion succeeds. `hclient-proto`
...
fuzz_target!(|data: &[u8]| {
    ...
    let Ok(first) = hclient_idn::domain_to_ascii(&domain) else {
    ...
    let second = hclient_idn::domain_to_ascii(&first)
