...
# for the streaming request body and its probe). Measured by running both
# engines, not inferred from the attribute count: several
# `#[wasm_bindgen_test]`s are cfg-gated, so the two numbers do not agree.
...
test-browser BROWSER="chrome":
    ...
    run_browser_suite crates/http-ng-fetch 78
