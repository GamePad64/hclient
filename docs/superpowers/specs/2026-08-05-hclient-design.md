...
### C13. `hclient-core` declares `Send` on two erased seams, so `Client` can stop being generic

The fifth class, and the first that touches the **core**. C12 puts a bound
on an opt-in call in the facade; this puts one on two traits in
`hclient-core` — which is what the no-`Send`-in-the-core rule exists to
stop. It is admitted because the rule's own stated reason does not reach
it.

That reason, from the guard script itself: *"erasing a type behind `dyn
Trait` drops auto-traits (amendment C1), so declaring the bound in the seam
forces it on backends that cannot satisfy it."* **Nothing here is the
seam.** `Transport` and `Timer` are untouched. `BoxedTransport`,
`BoxedTimer` and `ErasedInstant` are opt-in siblings, and a backend that
cannot produce a `Send` future simply does not implement one — its
inability is a compile error at the line that asked for an erased client,
not a bound forced on it from above.

**What it buys is the arity of the public API.** `Client<T, Tm>` and
`Response<ClientBody<T::Body, Tm>>` push both parameters onto every caller,
and a library that wants to stay generic pays five `where` lines and two
extra dependencies (`http-body`, `bytes`) for the privilege. Erased,
`hclient::Client` is one type, `Clone` with `Arc` semantics, and a library
writes `&Client`.

**Why the bound cannot be avoided.** `Transport::execute` is an `async fn`
in a trait, so its future is an RPITIT; naming its auto-traits for a
generic `T` needs return type notation, still `E0658` on rustc 1.98.0
(re-measured 2026-08-20). The way past is a trait with **no default body**,
where each backend writes `Box::pin(self.execute(req))` with `Self`
concrete and the proof available. That shape was proposed in
`docs/competitive-gaps.md` before this amendment existed, and that document
already judged it to obey decision D6: *"an additional trait beside the
seam, hand-written per backend, no proc macro and no `cfg` alias."*

**And why the timer needed a different trick.** `Timer::Instant` is
`Copy + PartialOrd`, and `Copy` on a trait object is not a thing. §G13
recorded that as permanent on the strength of it. It is not: the instant
stops crossing the boundary. `ErasedInstant` answers *how long ago was
this* and nothing else, so the instant stays inside the clock that made it.
Fixing `Instant` to a concrete type — what §G13 called the only way out —
is still impossible for the reason it gave, and is no longer needed.

The marker is `send-bound-exception: amendment-C13` and the sites are all
in `crates/hclient-core/src/unversioned/erased.rs`. The guard's allow-list
is a hardcoded set of amendment numbers, so admitting one is a visible act
rather than a wildcard — which is the property that makes this list worth
reading.
