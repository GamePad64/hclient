...
| claim | proof |
|---|---|
| The runtime seam is real | `crates/hclient/tests/two_runtimes.rs` — one generic function, `fetch_once<R>`, bounded only by `hclient_rt::{TcpConnect, Timer, Blocking} + Clone`, drives a real HTTP/1.1 exchange over loopback TCP under both `Tokio` and `Smol`. No `#[cfg]` in the test code; the file's single conditional is the gate excluding it from wasm targets, where its native dev-dependencies do not build. Plus `crates/hclient-native/tests/h1.rs`, which completes an exchange on a bare `futures` executor with no spawn and no timer |
| The delegation seam is real | `hclient-wasi` on `wasi:http` 0.3, where there is no socket to own at all — the transport delegates the whole exchange to the host |
...
`hclient-fetch` hardcodes `streaming_request_body = false` and
...
- `hclient-wasi`'s `full_duplex` and `request_trailers` (vertical 1)
- `hclient-native`'s `timeouts.connect` (vertical 2)
- `hclient-fetch`'s `streaming_request_body`, `full_duplex` and `redirects`
  (vertical 3)
...
(`crates/hclient-fetch/tests/transport.rs`), and a `RedirectPolicy` against a
`RedirectSupport::Internal` backend refused at `build()` and again on the
merged per-request value (`crates/hclient/tests/redirect.rs`,
...
`crates/hclient-fetch/tests/caps.rs`'s
...
> `hclient-native`'s `http2` feature; HTTP/3, v0.3, in its own crate
> `hclient-h3`, which could not have been that feature because it is bounded
...
> `hclient-tungstenite`, on its own trait pair rather than a method on
> `Transport`, so a backend that cannot do it is a compile error. hickory and
> DoH, as `hclient-dns-hickory` and `hclient-dns-doh`. Alt-Svc, v0.4 W1, as
> `hclient-select`'s slow tier — the HTTPS record is the fast one, and the
> order between them is a rule rather than an accident.

**Middleware was answered rather than built, and `hclient-tower` is the
...
written where the code is (`crates/hclient-dns-system/src/lib.rs`). Open, and
the only item on this list that is a plain inefficiency rather than a missing
capability.

**`hclient-rmcp` does not exist**, and it is named in three design documents
...
> (`crates/hclient/tests/deadline.rs`); the one case it does not cover — a
...
real consumer is `hclient-rmcp`, in v0.2.

> Closed, and by tests rather than by the consumer this paragraph was
> waiting for. Native streams since v0.2 W6 and `hclient-h3` since v0.3,
...
> **causally** rather than by a clock: in `crates/hclient-h3/tests/streaming.rs`
...
> `hclient-wasi` declares `streaming_request_body = true` and hands a
> `Streaming` body straight to the host; native no longer buffers; and
> `hclient-fetch`'s answer is behavioural rather than a flat refusal —
> whatwg/fetch#1470, so it depends on the browser. The backend that reports
> `false` today is `hclient-urlsession`, which arrived after this list was
> written and says so in the error a `Streaming` body gets.

Cross-backend cancellation is asymmetric. Only `hclient-fetch` cancels the
...
> the client: `crates/hclient-native/tests/cancel.rs`,
